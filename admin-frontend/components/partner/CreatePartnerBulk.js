...
const CreatePartnerBulk = ({ baseURL, maxCount = 30, onSuccess, onError, setIsLoading }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const csvFile = new File([csvBlob], 'partners.csv', { type: 'text/csv' })
      
      formData.append('file', csvFile)
      
      const res = await api.post(`/partner/bulk-upload`, formData, {
        headers: {
      ...
      if (res.data) {
        const { created, skipped, failed, total, partners } = res.data.summary || res.data
        
        setCreatedUsers(partners || [])
        ...
        if (onError) {
          onError(new Error('Something went wrong while creating partners'))
        } else {
          toast.error('Something went wrong while creating partners')
        }
      }
    } catch (error) {
      if (onError) {
        ...
        toast.error(error.response?.data?.message || 'Failed to create partners')
      }
    } finally {
      setProcessing(false)
  ...
  const downloadResultUsers = (e) => {
    ...
    element.download = `partner_credentials_${new Date().toISOString().slice(0, 10)}.csv`
  ...
  const downloadTemplate = (e) => {
    ...
    const sample = [
      {
        ...
        phone: "9876543210",
        address: "123 Partner Street, City, State",
        pincode: "123456",
      ...
      },
      {
        ...
        phone: "8765432109",
        address: "456 Partner Avenue, Town, State",
        pincode: "654321",
        desc: "Innovative business consulting firm"
      }
    ]
    ...
    const element = document.createElement('a')
    element.href = URL.createObjectURL(sample_csv_blob)
    element.download = `partner_bulk_template.csv`
        document.body.appendChild(element)
        element.click()
    document.body.removeChild(element)
  }
    ...
    parse(file, {
      ...
      complete: (result) => {
        ...
        if (result.data.length > maxCount) {
          const errorMsg = `Maximum ${maxCount} partners allowed. Found ${result.data.length}`
          toast.error(errorMsg)
          setUploadStatus({ success: false, error: errorMsg })
          return
        }
        ...
        toast.success(`Successfully uploaded ${cleanedData.length} partners`)
      },
      error: (error) => {
        setUploading(false)
    ...
    <div className="w-full max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <p className="text-gray-600 mb-4 text-sm">
          Upload a CSV file to create multiple partners at once. Passwords will be automatically generated and included in the result file.
        </p>
        ...
        <div className="bg-blue-50 border-l-4 border-blue-400 p-4 rounded-r-lg">
          ...
          <ul className="text-blue-800 space-y-1 text-xs">
            <li>• Maximum <strong>{maxCount} partners</strong> per upload</li>
            ...
            <div className="flex items-center justify-center gap-3">
              ...
              <div>
                <p className="text-green-600 font-medium text-sm">
                  Successfully uploaded {users.length} partners
                </p>
                <p className="text-gray-500 text-xs">
                  Ready to create partners
                </p>
              </div>
            </div>
          )}
          ...
          <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-green-800 font-medium text-sm">
                Successfully created {createdUsers.length} partners!
              </p>
            </div>
...
export default CreatePartnerBulk 