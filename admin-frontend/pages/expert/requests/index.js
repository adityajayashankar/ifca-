...
const [error, setError] = useState(null);
  ...
  try {
    let url = "";
    if (user?.userType === "partner" || user?.userType === "expert") {
      const base = "/comRequest";
      if (type === "pending") {
        url = `${base}/pending/${user.userType}/${user.unifiedUser.id}`;
      } else if (type === "approved") {
        url = `${base}/approved/${user.userType}/${user.unifiedUser.id}`;
      } else if (type === "all") {
        url = `${base}/all/${user.userType}/${user.unifiedUser.id}`;
      }
    } else {
      if (type === "pending") {
    ...
    if (type === "pending") setPendingReq(res.data.data || []);
    if (type === "approved") setApprovedReq(res.data.data || []);
    if (type === "all") setAllReq(res.data.data || []);
  } catch (err) {
    setError(err);
  }
  setLoading(false);
 ...
 useEffect(() => {
...
}, [viewType, user?.userType, user?.unifiedUser?.id]);
  ...
  setExpandedRows({});
  // fetchRequests will run automatically via useEffect
};
