...
const CreateCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let response;
      if (id) {
        response = await api.put(`/course/courses/${id}`, payload);
        router.push(`/admin/courses/${id}`);
      } else {
        response = await api.post('/course/courses', payload);
        // Fetch the new course details to get moodleCourseId
        const courseDetails = await api.get(`/course/courses/${response.data.id}`);
        const moodleCourseId = courseDetails.data.moodleCourseId;
        // Redirect to details page with openMoodleSettings flag
        router.push(`/admin/courses/${response.data.id}?openMoodleSettings=1&moodleCourseId=${moodleCourseId}`);
      }
    } catch (error) {
    ...
    <div className="min-h-screen w-full bg-gray-50 flex flex-col">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="w-full px-4 py-3 flex items-center space-x-2 text-sm">
          ...
          <ChevronRightIcon className="w-4 h-4 text-gray-400" />
          <span className="text-gray-900 font-semibold">{id ? 'Edit Course' : 'Create Course'}</span>
        </div>
      </div>
      <div className="flex-1 flex flex-col justify-center items-center w-full px-4 py-8 overflow-auto">
        <div className="mb-10 flex flex-col items-center justify-center">
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-2 tracking-tight flex items-center gap-3">
            <AddIcon className="text-orange-500 text-5xl" />
            {id ? 'Edit Course' : 'Create Course'}
          </h1>
          <p className="text-lg text-gray-500 font-medium">Fill in the details below to {id ? 'edit' : 'create'} your course</p>
        </div>
