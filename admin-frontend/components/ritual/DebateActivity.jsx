...
export default function DebateActivity({ activity, activityData = {}, isModerator }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (err?.response?.status === 404) setServerUnsupported(true);
      }
  ...
  const handleAddArgument = async () => {
    ...
    const optimistic = { ...payload, id: Date.now().toString(), timestamp: new Date().toISOString() };
    ...
    try {
      ...
      if (error.response?.status === 404) toast.info('Argument saved locally (server endpoint missing)');
      else toast.error('Failed to add argument');
    }
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-900">
      <div className="bg-gray-800 px-6 py-4 border-b border-gray-700">
        ...
        {activityData.description && <p className="text-gray-300">{activityData.description}</p>}
        {activityData.instructions && <p className="text-gray-400 text-sm mt-2">{activityData.instructions}</p>}
      </div>
      ...
      <div className="flex-1 p-6 overflow-auto grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gray-800 rounded-lg p-4 flex flex-col">
          ...
          <div className="flex-1 overflow-auto space-y-3">
            {forArgs.length === 0 ? <p className="text-gray-400">No arguments yet. Add one!</p> : forArgs.map((a, i) => {
        ...
        <div className="bg-gray-800 rounded-lg p-4 flex flex-col">
          ...
          <div className="flex-1 overflow-auto space-y-3">
            {againstArgs.length === 0 ? <p className="text-gray-400">No arguments yet. Add one!</p> : againstArgs.map((a, i) => {
              const key = a?.id || a?.timestamp || (typeof a === 'string' ? a : null) || i;
              return (
                <div key={key} className="bg-gray-700 p-3 rounded">
                  <div className="text-sm text-white font-semibold">{a?.userName}</div>
      ...
      <div className="bg-gray-800 px-6 py-4 border-t border-gray-700 flex items-center gap-3">
        ...
        <input value={newArg} onChange={(e) => setNewArg(e.target.value)} placeholder="Add your argument..." className="flex-1 px-4 py-2 bg-gray-700 text-white rounded-lg focus:ring-2 focus:ring-orange-500 focus:outline-none" />
