...
import { MdChevronRight, MdHome, MdAdd, MdDelete, MdEdit, MdVisibility, MdSearch, MdMoreVert, MdShare, MdContentCopy, MdDescription, MdShortText, MdRadioButtonChecked, MdCheckBox, MdArrowDropDownCircle, MdPerson, MdCalendarToday, MdAccessTime } from "react-icons/md";
...
const AdminFormResponses = () => {
  ...
  // Fetch forms with pagination and filters
  const fetchForms = async (page = 1, search = "", sortBy = "createdAt", sortOrder = "desc") => {
    try {
      setLoading(true);
      ...
      const { data: forms, pagination: paginationData } = response.data;
  ...
  useEffect(() => {
    const sortMapping = {
      'az': { sortBy: 'formName', sortOrder: 'asc' },
      'za': { sortBy: 'formName', sortOrder: 'desc' },
      'recent': { sortBy: 'createdAt', sortOrder: 'desc' },
      'oldest': { sortBy: 'createdAt', sortOrder: 'asc' }
    };

    const { sortBy, sortOrder } = sortMapping[sortOption] || sortMapping.recent;
    fetchForms(1, searchQuery, sortBy, sortOrder);
  ...
  const handlePageChange = (newPage) => {
    ...
    const { sortBy, sortOrder } = sortMapping[sortOption] || sortMapping.recent;
    fetchForms(newPage, searchQuery, sortBy, sortOrder);
  };
  ...
  const confirmDeleteForm = async () => {
    const formToDelete = data.find(form => form.id === deleteFormId);
    ...
    try {
      ...
      const { sortBy, sortOrder } = sortMapping[sortOption] || sortMapping.recent;
      fetchForms(pagination.page, searchQuery, sortBy, sortOrder);
    ...
    const exampleForms = [
      {
        ...
        questions: [
          { question: 'How satisfied are you?', answerType: 'radio', description: 'Rate your satisfaction', isRequired: true, options: ['Very satisfied', 'Satisfied', 'Neutral', 'Dissatisfied'] },
          { question: 'What can we improve?', answerType: 'text', description: 'Share your suggestions', isRequired: false },
        ],
      },
      {
        formName: 'Event Registration',
        questions: [
          { question: 'Full Name', answerType: 'text', description: 'Enter your full name', isRequired: true },
          { question: 'Email Address', answerType: 'text', description: 'Enter your email', isRequired: true },
          { question: 'Will you attend?', answerType: 'radio', description: 'Confirm your attendance', isRequired: true, options: ['Yes', 'No', 'Maybe'] },
        ],
      },
      {
        formName: 'Employee Survey',
        questions: [
          { question: 'Department', answerType: 'text', description: 'Your department', isRequired: true },
          { question: 'How do you rate your work environment?', answerType: 'radio', description: 'Rate your work environment', isRequired: true, options: ['Excellent', 'Good', 'Average', 'Poor'] },
        ],
      },
    ];
  ...
  const handleFormView = (formId) => {
    router.push(`/admin/forms/create/${formId}`);
  };
  ...
  const exampleForms = [
    {
      ...
      questions: [
        { question: 'How satisfied are you?', answerType: 'radio', options: ['Very satisfied', 'Satisfied', 'Neutral', 'Dissatisfied'] },
        { question: 'What can we improve?', answerType: 'text' },
      ],
    },
    {
      formName: 'Event Registration',
      ...
      questions: [
        { question: 'Full Name', answerType: 'text' },
        { question: 'Email Address', answerType: 'text' },
        { question: 'Will you attend?', answerType: 'radio', options: ['Yes', 'No', 'Maybe'] },
      ],
    },
    {
      formName: 'Employee Survey',
      ...
      questions: [
        { question: 'Department', answerType: 'text' },
        { question: 'How do you rate your work environment?', answerType: 'radio', options: ['Excellent', 'Good', 'Average', 'Poor'] },
      ],
    },
  ];
  ...
  // Helper for type icon and label
  const typeIconAndLabel = (type) => {
    switch(type) {
      case 'text': return { icon: <MdShortText className="text-orange-400" />, label: 'Short answer' };
      case 'radio': return { icon: <MdRadioButtonChecked className="text-orange-400" />, label: 'Multiple choice' };
      case 'checkbox': return { icon: <MdCheckBox className="text-orange-400" />, label: 'Checkboxes' };
  ...
  return (
    <>
      <Head>
        <title>Forms Management - IFCA Admin</title>
        {/* Open Graph meta tags for better social media sharing */}
        <meta property="og:title" content="Forms Management - IFCA Admin" />
        <meta property="og:description" content="Manage and create forms for IFCA communities" />
        ...
        <meta name="twitter:title" content="Forms Management - IFCA Admin" />
        <meta name="twitter:description" content="Manage and create forms for IFCA communities" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {filteredData.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="relative flex flex-col bg-white rounded-2xl shadow group transition-all duration-200 border border-gray-100 hover:shadow-2xl hover:border-orange-400 min-h-[200px] overflow-visible"
                        >
                          {/* Orange accent bar */}
                          ...
                          <div className="flex-1 flex flex-col pl-5 pr-3 pt-4 pb-3">
                            ...
                            <div className="flex items-start justify-between mb-2">
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-2 mt-1 mb-2">
                                  ...
                                  <span className="text-xs text-gray-600 font-medium">{creatorName}</span>
                                ...
                                <div className="flex flex-wrap gap-2 mb-2">
                                  ...
                                  {uniqueTypes.length > 3 && (
                                    <span className="inline-flex items-center px-2 py-0.5 bg-gray-50 text-gray-600 rounded-full text-xs font-semibold">
                                ...
                                {communities.length > 0 && (
                                  <div className="flex flex-wrap gap-2 mb-2">
                                    {communities.slice(0, 2).map((title, idx) => (
                              ...
                              <div className="relative ml-2">
                                ...
                                {menuOpenId === item.id && (
                                  <div ref={menuRef} className="absolute right-0 mt-2 w-40 bg-white border border-gray-200 rounded-xl shadow-lg z-[100]">
                                    ...
                                    <button
                                      onClick={() => { handleFormView(item.id); setMenuOpenId(null); }}
                                      className="w-full flex items-center gap-2 px-4 py-2 text-blue-600 hover:bg-blue-50"
                                    >
                                      <MdEdit className="text-lg" /> Edit
                                    </button>
                                    <button
                                      onClick={() => { formDeleteHandler(item.id); setMenuOpenId(null); }}
                            ...
                            <div className="flex gap-2 mb-3">
                              <span className="px-2 py-0.5 bg-orange-100 text-orange-600 rounded-full text-xs font-semibold">
                                {item._count?.FormResponse || 0} Responses
                              </span>
                              <span className="px-2 py-0.5 bg-orange-100 text-orange-600 rounded-full text-xs font-semibold">
                                {item._count?.Question || 0} Questions
                              </span>
                              {item.isGlobal && (
                            ...
                            <div className="flex justify-end items-center gap-2 mt-auto">
                              ...
                              <button
                                onClick={() => handleFormView(item.id)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md mx-auto flex flex-col gap-6 relative">
            ...
            <div 
              ...
              onClick={() => {
                const formName = data.find(form => form.id === deleteFormId)?.formName;
                if (formName) {
                  navigator.clipboard.writeText(formName);
                }
              }}
            >
              {data.find(form => form.id === deleteFormId)?.formName || ''}
            </div>
            <input
              type="text"
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                className={`relative overflow-hidden group bg-gradient-to-r from-pink-500 to-orange-500 hover:from-orange-600 hover:to-pink-500 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 shadow focus:ring-2 focus:ring-pink-300 ${deleteInput !== data.find(form => form.id === deleteFormId)?.formName ? 'cursor-not-allowed opacity-60' : ''}`}
                onClick={confirmDeleteForm}
                disabled={deleteInput !== data.find(form => form.id === deleteFormId)?.formName}
              >
