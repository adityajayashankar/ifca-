...
const FormView = () => {
  ...
  const updateFormData = async (e) => {
    ...
    try {
      const response = await api.put(`/forms/${formId}`, {
        ...
        isGlobal: formData.selectedCommunityIds && formData.selectedCommunityIds.length === 0,
      });
      toast.success("Form updated successfully!", {
        position: "top-right",
        autoClose: 5000,
      });
      // If community-specific, redirect to comHome/:communityId?tab=6
      if (formData.selectedCommunityIds && formData.selectedCommunityIds.length > 0) {
        const firstCommunityId = formData.selectedCommunityIds[0];
        router.push(`/comHome/${firstCommunityId}?tab=6`);
      } else {
  ...
  return (
    <>
      <Head>
        <title>{formData?.formName || 'Form'} - IFCA Admin</title>
        {/* Open Graph meta tags for better social media sharing */}
        <meta property="og:title" content={formData?.formName || 'Form'} />
        <meta property="og:description" content={`Fill out this form: ${formData?.formName || 'Form'}`} />
        ...
        <meta name="twitter:title" content={formData?.formName || 'Form'} />
        <meta name="twitter:description" content={`Fill out this form: ${formData?.formName || 'Form'}`} />
