...
function HuddlesHomePage() {
  const router = useRouter();
  const user = useSelector(selectUser);
  const [searchQuery, setSearchQuery] = useState("");
  const [filter, setFilter] = useState("");
  const [sort, setSort] = useState("");
  const [allHuddles, setAllHuddles] = useState([]); // Store ALL huddles
  const [filteredHuddles, setFilteredHuddles] = useState([]); // Filtered and grouped huddles
  const [paginatedHuddles, setPaginatedHuddles] = useState([]); // Paginated subset of filtered huddles
  ...
  // Fetch all huddles on mount (no pagination from API)
  useEffect(() => {
    fetchAllHuddles();
  }, []);

  const fetchAllHuddles = async () => {
    try {
      setLoading(true);
      // Fetch ALL huddles without pagination for filtering/grouping
      const response = await api.get(`/huddle/all`, {
        params: {
          page: 1,
          limit: 10000 // Large limit to get all huddles
        }
      });
      if (response.data && response.data.huddles) {
        setAllHuddles(response.data.huddles);
      }
    } catch (error) {
  ...
  // Filter and group huddles immediately when data or filters change
  useEffect(() => {
    if (allHuddles && allHuddles.length > 0) {
      let filtered = allHuddles.filter(huddle =>
  ...
  }, [searchQuery, filter, sort, allHuddles, activeTab]);

  // Apply pagination to filtered huddles
  useEffect(() => {
    if (filteredHuddles && filteredHuddles.length > 0) {
      const startIndex = (currentPage - 1) * itemsPerPage;
      const endIndex = startIndex + itemsPerPage;
      setPaginatedHuddles(filteredHuddles.slice(startIndex, endIndex));
    } else {
      setPaginatedHuddles([]);
    }
  }, [filteredHuddles, currentPage, itemsPerPage]);

  const handleAddHuddle = () => {
    setLoadingAdd(true);
    setTimeout(() => {
      setLoadingAdd(false);
      router.push("/admin/huddle/create");
    }, 400);
  };
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between mb-6">
            ...
            <div className="flex gap-2 bg-gray-100 p-1 rounded-lg">
              ...
              >
                Upcoming ({allHuddles.filter(h => h.isScheduled && !h.isLive).length})
              </button>
              <button
              ...
              >
                Live ({allHuddles.filter(h => h.isLive).length})
              </button>
              <button
              ...
              >
                Completed ({allHuddles.filter(h => !h.isScheduled && !h.isLive && h.endTime).length})
              </button>
            </div>
          ...
          ) : paginatedHuddles?.length > 0 ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4">
                {paginatedHuddles.map((huddle, index) => {
                          ...
                          <div className="space-y-2 mb-3 flex-shrink-0">
                            ...
                            {huddle.locationType && (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-gray-700 capitalize">{huddle.locationType.toLowerCase()}</span>
                              </div>
                            )}
                          </div>
              ...
              {/* Pagination Controls */}
              {filteredHuddles.length > itemsPerPage && (
                <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-gray-200">
                  <div className="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <select
                      ...
                      className="border border-gray-300 rounded-md px-2 py-1 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                    >
                    ...
                    <span>of {filteredHuddles.length} huddles</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      {(() => {
                        const totalPages = Math.ceil(filteredHuddles.length / itemsPerPage);
                    ...
                    <button
                      onClick={() => setCurrentPage(prev => Math.min(Math.ceil(filteredHuddles.length / itemsPerPage), prev + 1))}
                      disabled={currentPage >= Math.ceil(filteredHuddles.length / itemsPerPage)}
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                        currentPage >= Math.ceil(filteredHuddles.length / itemsPerPage)
                          ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                          : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
                      }`}
                    >
