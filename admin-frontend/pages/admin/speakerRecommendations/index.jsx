  ...
  useEffect(() => {
    const getMyRecommendations = async () => {
      try {
        ...
        setMyRec(res.data.recommendations);
      } catch (err) {
  ...
  return (
    <div className="mt-5 ml-10">
        <div className='input__group__header'>
            <h2 className="">Speaker Recommendations</h2>
        </div>
        <div className="mt-10 flex flex-wrap gap-5">
            {myRec?.map((item, index) => (
                <div className="border-2 border-gray-200 rounded-xl p-5" key={index}>
                    ...
                    <div className="my-5"><CheckCircleIcon className='text-green-500' /> Sent by <Link href={`/admin/speakerRecommendations/${item.speakerDetails.id}`}><span className="underline text-orange-500">{item.speakerDetails.name}</span></Link></div>
                </div>
            ))}
        </div>
    </div>
