...
export default function RitualPage() {
  ...
  const broadcastActivityCompleted = useCallback(async (activityId, isCompleted) => {
  ...
  }, [hmsActions, isModerator]);

  // Toggle activity completion (for hosts - can mark complete/incomplete)
  const toggleActivityComplete = useCallback(async (activityId) => {
  ...
  }, [id, user, isModerator, broadcastActivityCompleted]);
  ...
  }, [id, user, isModerator, toggleActivityComplete]);

  // Fetch community role and determine if user is moderator (combined to prevent circular deps)
  useEffect(() => {
    if (!huddle || !user || !huddle.communityId) return;

    const fetchAndSetRole = async () => {
      try {
        const response = await api.get(`/community/${huddle.communityId}/my-role`);
        const role = response.data.success && response.data.data ? response.data.data.role : null;
        
        // Set community role
        setCommunityRole(role);
        
        // Determine if user is moderator based on role and huddle position
        ...
        const isCommunityAdmin = role === 'ADMIN';
        const isCommunityModerator = role === 'MODERATOR';
        
        setIsModerator(isLeader || isCreator || isCommunityAdmin || isCommunityModerator);
      } catch (error) {
        console.error('Error fetching community role:', error);
        setCommunityRole(null);
        
        // Still set moderator based on huddle position alone
        const userUnifiedId = user?.unifiedUserId || user?.id;
        const isCreator = huddle.creatorId === parseInt(userUnifiedId);
        const isLeader = huddle.leaderId === parseInt(userUnifiedId);
        setIsModerator(isLeader || isCreator);
      }
    };
  ...
  useEffect(() => {
    ...
    // Reset initialization flag when loading new huddle
    ...
    const fetchHuddle = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setIsLive(huddleData.isLive || false);
          
          // Fetch activities
          const activitiesResponse = await api.get(`/huddle/${id}/activities`);
          if (activitiesResponse.data.success) {
            setActivities(activitiesResponse.data.activities || []);
            // Don't set currentActivityIndex here - let URL handle it
          }
          ...
          if (huddleData.isLive && huddleData.roomId && user && !hasManuallyLeftRef.current) {
  ...
  useEffect(() => {
    ...
    let initialIndex = 0; // Default to first activity
  ...
  const join100msRoom = async (roomId) => {
    ...
    if (isJoiningRef.current) {
      console.log('Frontend: Already joining, skipping duplicate call');
      return;
    }
    
    // Check if already joined
    if (localPeer && hmsToken) {
      console.log('Frontend: Already joined to room');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        if (joinResponse.data.success) {
          console.log('Frontend: Successfully recorded huddle attendance');
          // Don't immediately refresh - let 100ms peers update naturally or wait for next poll
        }
      } catch (joinError) {
        // If already joined, that's okay - continue
        console.warn('Frontend: Could not record attendance (might already be joined):', joinError.response?.data?.message || joinError.message);
      }
  ...
  useEffect(() => {
    ...
    allMessages.forEach((message) => {
      ...
      try {
        ...
        // DON'T update isModerator here - it causes infinite loops
        // Moderator status should be determined only from huddle data, not 100ms role
        // if (isHostRole) {
        //   setIsModerator(true);
        // } else {
        ...
        if (messageData.type === ACTIVITY_CHANGE && typeof messageData.activityIndex === 'number') {
          ...
          if (newIndex !== currentActivityIndex && newIndex >= 0 && newIndex < activities.length) {
            ...
            console.log(`Activity synchronized to index ${newIndex} by host`);
          }
        }
  ...
  }, [allMessages, localPeer, activities.length, hmsActions]);

  // 100ms notifications - memoized with useCallback to prevent unnecessary re-triggers
  useEffect(() => {
  ...
  useEffect(() => {
    // If huddle already ended, don't auto-start
    if (!huddle || huddle?.endTime || isLive) {
      setTimeUntilStart(null);
      return;
    }

    const calculateTimeUntilStart = async () => {
      const now = moment();
      ...
      if (diff > 0) {
        ...
        setTimeUntilStart(null);
        if (!isLive) {
          // Call launch directly without dependency on handleLaunch
          try {
  ...
  const handleJoin = async () => {
    try {
      ...
      // Reset manual leave flag when user explicitly joins
      hasManuallyLeftRef.current = false;
      
      // REMOVED: Invitation check for testing - allow everyone to join
      // if (huddle && huddle.creatorId !== parseInt(userUnifiedId)) {
      //   try {
      //     const invitationCheck = await api.get(`/huddle/user/${userUnifiedId}/invitations`);
      //     const userInvitation = invitationCheck.data.invitations?.find(
      //       inv => inv.huddleId === parseInt(id)
      //     );
      //     
      //     if (userInvitation && userInvitation.status === 'PENDING') {
      //       toast.error('Please accept the invitation first');
      //       router.push('/ritual/invitations');
      //       return;
      //     }
      //   } catch (err) {
      //     console.log('Could not check invitation status:', err);
      //   }
      // }
      ...
      if (response.data.success) {
        ...
        // Join 100ms room if available (regardless of isLive status for testing)
        if (huddle?.roomId) {
          await join100msRoom(huddle.roomId);
        } else {
      ...
      // REMOVED: Auto-redirect for testing
      // if (errorMessage.includes('invitation') || errorMessage.includes('invited')) {
      //   setTimeout(() => {
      //     router.push('/ritual/invitations');
      //   }, 2000);
      // }
  ...
  const handleLeaveConfirm = async () => {
    try {
      ...
      await api.post(`/huddle/${id}/leave`, { userId: userUnifiedId });
        console.log('Frontend: Successfully left huddle');
      } catch (leaveError) {
        // If user wasn't in attendance, that's okay - they might have joined 100ms but not recorded attendance
        if (leaveError.response?.status === 404 && leaveError.response?.data?.message?.includes('not in this huddle')) {
          console.warn('Frontend: User not in attendance records, but leaving 100ms room anyway');
        } else {
          throw leaveError; // Re-throw if it's a different error
        }
      }
      ...
      // Navigate back to community huddles page
      if (huddle?.communityId) {
        router.push(`/comHome/${huddle.communityId}?tab=huddles`);
      } else {
  ...
  const handleNextActivity = async () => {
    ...
    if (currentActivityIndex < activities.length - 1) {
      ...
      // Only broadcast if the index actually changed and broadcast function exists
  ...
  const calculateVideoOnlyGrid = () => {
    ...
    if (count === 0) return { cols: 1, rows: 1, tileWidth: '100%', tileHeight: '100%' };
    if (count === 1) return { cols: 1, rows: 1, tileWidth: '100%', tileHeight: '100%' };
    if (count === 2) return { cols: 2, rows: 1, tileWidth: '50%', tileHeight: '100%' };
    if (count === 3) return { cols: 2, rows: 2, tileWidth: '50%', tileHeight: '50%' };
    if (count === 4) return { cols: 2, rows: 2, tileWidth: '50%', tileHeight: '50%' };
    if (count <= 6) return { cols: 3, rows: 2, tileWidth: '33.33%', tileHeight: '50%' };
    if (count <= 9) return { cols: 3, rows: 3, tileWidth: '33.33%', tileHeight: '33.33%' };
    if (count <= 12) return { cols: 4, rows: 3, tileWidth: '25%', tileHeight: '33.33%' };
    if (count <= 16) return { cols: 4, rows: 4, tileWidth: '25%', tileHeight: '25%' };
    if (count <= 20) return { cols: 5, rows: 4, tileWidth: '20%', tileHeight: '25%' };
    if (count <= 25) return { cols: 5, rows: 5, tileWidth: '20%', tileHeight: '20%' };
          ...
          <div className="flex items-center gap-2 md:gap-4 ml-2">
            ...
            {localPeer && peers && (
              ...
              >
                ...
                <span className="text-xs md:text-sm font-semibold">{peers.length}</span>
                {tilesExpanded ? (
                  ...
                  <div
                    ...
                    onClick={() => {
                      try {
                        ...
                        // Broadcast change if user is moderator/host
                        if (typeof broadcastActivityChange === 'function' && newIndex !== currentActivityIndex) {
                          broadcastActivityChange(newIndex);
                        }
                      } catch (err) {
              ...
              {peers && peers.length === 0 ? (
                ...
                <div className="w-full h-full relative overflow-hidden bg-black flex items-center justify-center">
                  {(() => {
                    ...
                    return (
                      <div
                        className="grid w-full h-full overflow-y-auto"
                        style={{
                    ...
                    <>
                      <div className="flex items-center justify-between px-2 md:px-4 py-1.5 md:py-2 border-b border-gray-800">
                        <div className="flex items-center gap-2">
                          <span className="text-white text-xs md:text-sm font-medium">Participants ({peers.length})</span>
                          <button
                      ...
                      <div className="relative">
                        <div 
                          ref={peerScrollRef}
                          className="overflow-x-auto scrollbar-hide p-2 md:p-4 flex gap-2 md:gap-4"
                          style={{ scrollBehavior: 'smooth' }}
                        >
                          {peers.map((peer) => (
                            <div key={peer.id} className="flex-shrink-0 w-[140px] md:w-[180px]">
                              <VideoSpaces
                                isLocal={false}
                                peer={peer}
                                setPinned={setPinned}
                ...
                {isModerator && (
                  <div className="absolute top-4 right-4 z-10 flex items-center gap-1 md:gap-2">
                    ...
                    <button
                      ...
                      title="Activity Only"
                    >
                      <span className="hidden md:inline">Activity Only</span>
                    ...
                    <button
                      ...
                      title="Video Only"
                    >
                      <span className="hidden md:inline">Video Only</span>
                      <span className="md:hidden">Video</span>
                    </button>
                    <button
                      onClick={handlePreviousActivity}
                      disabled={currentActivityIndex === 0 || !isModerator}
                      className="p-1.5 md:p-2 bg-gray-700 bg-opacity-90 backdrop-blur-sm text-white rounded hover:bg-gray-600 disabled:opacity-50"
                      title={!isModerator ? 'Only moderators can switch activities' : 'Previous activity'}
                    >
                      <MdSkipPrevious className="w-4 h-4 md:w-5 md:h-5" />
                    </button>
                    <button
                      onClick={handleNextActivity}
                      disabled={currentActivityIndex === activities.length - 1 || !isModerator}
                      className="p-1.5 md:p-2 bg-gray-700 bg-opacity-90 backdrop-blur-sm text-white rounded hover:bg-gray-600 disabled:opacity-50"
                      title={!isModerator ? 'Only moderators can switch activities' : 'Next activity'}
                    >
                      <MdSkipNext className="w-4 h-4 md:w-5 md:h-5" />
                    </button>
                  </div>
                )}
                <div ref={presentationRef} className="flex-1 flex items-center justify-center overflow-hidden bg-black h-full min-h-0">
                  {renderActivity()}
                </div>
              </div>
            </div>
