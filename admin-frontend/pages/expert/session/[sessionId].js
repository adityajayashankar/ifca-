...
import { oneSession, setOneSession } from "@/store/features/session";
...
function ExpertSession() {
  ...
  const handleJoinSession = (e) => {
    e.preventDefault();
    const newTab = window.open(`/expert/room/${selectedSession?.roomId}`, '_blank');
    newTab.focus();
  };
          ...
          {view === "home" ? (
            <section className="col-span-6 flex flex-col gap-8 p-2 items-center md:items-start justify-center md:justify-start ml-8 md:ml-20">
              ...
              {selectedSession?.SessionSlot[0]?.isOnline && (
                <button
                  className="btn btn-green font-bold w-64"
              ...
              <button
                className="btn btn-blue font-bold w-64"
                onClick={() =>
                  router.push(
                    `/admin/session/create/${router.query["sessionId"]}`
                  )
                }
              >
                Edit session
