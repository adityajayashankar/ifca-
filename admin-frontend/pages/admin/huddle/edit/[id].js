...
function EditHuddlePage() {
  ...
  useEffect(() => {
    if (formData.communityId && formData.leaderSelectionType === 'USER') {
      fetchCommunityMembers();
    }
  }, [formData.communityId, formData.leaderSelectionType]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        // Only navigate if not already on the target page
        const targetPath = `/admin/huddle/${id}`;
          ...
          <div className="bg-white rounded-xl shadow-md p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="flex items-center justify-end gap-4 pt-6 border-t">
                <button
                  type="button"
                  onClick={() => router.push(`/admin/huddle/${id}`)}
