...
import { selectAllPartners, setAllPartners, setPartner } from "@/store/features/partnerSlice"
...
const PartnerPage = () => {
  ...
  const [enableDialog, setEnableDialog] = useState({ open: false, partnerId: null, partnerName: "" })
  const [enablingPartner, setEnablingPartner] = useState(null)
  const [exporting, setExporting] = useState(false)
  const [disableDialog, setDisableDialog] = useState({ open: false, partnerId: null, partnerName: "" })
  const [disablingPartner, setDisablingPartner] = useState(null)
  ...
  // Extract active and inactive partners from the new API response structure
  const activePartners = partners?.activePartners || []
  const inactivePartners = partners?.inactivePartners || []
  const currentPartners = activeTab === 0 ? activePartners : inactivePartners

  // Check if data is loaded
  const isDataLoaded = Array.isArray(activePartners) && Array.isArray(inactivePartners)

  // Filter and sort partners
  const filtered = currentPartners
  ...
  const handleEnableClick = (partnerId, partnerName) => {
    setEnableDialog({ open: true, partnerId, partnerName })
  }

  const handleDisableClick = (partnerId, partnerName) => {
    setDisableDialog({ open: true, partnerId, partnerName })
  }

  const handleEnableConfirm = async () => {
    const { partnerId } = enableDialog
    setEnablingPartner(partnerId)
    try {
      const res = await api.patch(`/partner/${partnerId}/enable`)
      if (res.status === 200 || res.status === 204) {
        dispatch(setAllPartners(user.id))
      } else {
        console.error("Failed to enable partner")
      }
    } catch (error) {
      console.error("Error enabling partner:", error)
    } finally {
      setEnablingPartner(null)
      setEnableDialog({ open: false, partnerId: null, partnerName: "" })
    }
  }

  const handleDisableConfirm = async () => {
    const { partnerId } = disableDialog
    setDisablingPartner(partnerId)
    try {
      const res = await api.delete(`/partner/${partnerId}`)
      if (res.status === 200 || res.status === 204) {
        dispatch(setAllPartners(user.id))
      } else {
        console.error("Failed to disable partner")
      }
    } catch (error) {
      console.error("Error disabling partner:", error)
    } finally {
      setDisablingPartner(null)
      setDisableDialog({ open: false, partnerId: null, partnerName: "" })
    }
  }
  ...
  const exportToExcel = () => {
    setExporting(true);
    try {
      const exportData = currentPartners.map((partner, index) => {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, `${activeTab === 0 ? 'Active' : 'Inactive'} Partners`);
      const timestamp = new Date().toISOString().slice(0, 19).replace(/:/g, '-');
      const filename = `partners_${activeTab === 0 ? 'active' : 'inactive'}_${timestamp}.xlsx`;
  ...
  const columns = [
      ...
      renderCell: (params) => (
        <Box sx={{ textAlign: "center" }}>
          <Box sx={{ display: "flex", flexDirection: "column", gap: 0.5 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Typography sx={{ fontSize: "12px", color: "text.secondary" }}>Created:</Typography>
              <Typography sx={{ fontSize: "12px", fontWeight: 600, color: "green.700" }}>
                {params.row.statistics?.communities?.created || 0}
              </Typography>
            </Box>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Typography sx={{ fontSize: "12px", color: "text.secondary" }}>Subscribed:</Typography>
              <Typography sx={{ fontSize: "12px", fontWeight: 600, color: "blue.700" }}>
                {params.row.statistics?.communities?.subscribed || 0}
              </Typography>
            </Box>
          </Box>
        </Box>
    ...
    },
    {
      ...
      renderCell: (params) => (
        ...
        >
          {params.row.desc || "No description available"}
        </Typography>
      ),
    },
    {
      ...
      renderCell: (params) => (
        <Box sx={{ display: "flex", gap: 1 }}>
          <Tooltip title="View Details">
            <IconButton
              size="small"
              onClick={() => router.push(`/admin/partner/${params.row.partnerId}`)}
              sx={{
                color: "orange.700",
                "&:hover": {
                  bgcolor: "orange.50",
          ...
          <Tooltip title="Edit Partner">
            <IconButton
              size="small"
              onClick={() => handleEditPartner(params.row.partnerId)}
              sx={{
                color: "orange.700",
                "&:hover": {
                  bgcolor: "orange.50",
          ...
          {activeTab === 0 && (
            <Tooltip title="Disable Partner">
              <IconButton
                size="small"
                onClick={() => handleDisableClick(params.row.partnerId, params.row.name)}
                disabled={disablingPartner === params.row.partnerId}
                sx={{
                  color: "red.700",
                  "&:hover": {
                    bgcolor: "red.50",
              ...
              >
                {disablingPartner === params.row.partnerId ? (
                  <svg
          ...
          {activeTab === 1 && (
            <Tooltip title="Enable Partner">
              <IconButton
                size="small"
                onClick={() => handleEnableClick(params.row.partnerId, params.row.name)}
                disabled={enablingPartner === params.row.partnerId}
                sx={{
                  color: "green.700",
                  "&:hover": {
                    bgcolor: "green.50",
              ...
              >
                {enablingPartner === params.row.partnerId ? (
                  <svg
                    className="animate-spin h-4 w-4"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
  ...
  // Transform partners data to include row id and statistics
  const rows =
  ...
  const handleEditPartner = async (partnerId) => {
    try {
      setLoadingAdd(true);
      // Navigate directly to edit page - it will fetch its own data
      router.push(`/admin/partner/add/${partnerId}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50">
      ...
      <div className="max-w-[1920px] mx-auto px-4 overflow-x-auto">
        {/* Header Section - All in one row with tabs first */}
        <div className="bg-white rounded-xl shadow-sm px-6 py-4 mt-4">
          <div className="flex flex-col xl:flex-row gap-4 items-start xl:items-center justify-between">
            {/* Left side - Tabs, Title, Search, Sort */}
            <div className="flex flex-col lg:flex-row gap-4 lg:items-center flex-1">
              ...
              <div className="flex bg-gray-100 rounded-lg p-1">
                ...
                <button
                  onClick={() => handleSectionChange("inactive")}
                  className={`px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${
        ...
        <Paper
          ...
          sx={{
            ...
            border: "1px solid",
            borderColor: "divider",
            height: "calc(100vh - 280px)", // Reduced height to prevent scrolling
          }}
        >
          <DataGrid
            rows={rows}
          ...
          {(!currentPartners || currentPartners.length === 0) && (
            ...
            >
              ...
              >
                {activeTab === 0 ? "No Active Partners Yet" : "No Inactive Partners"}
              </Typography>
            </Box>
          )}
        </Paper>

        {/* Enhanced Enable Partner Confirmation Dialog */}
        <Dialog
        ...
        >
          ...
          <DialogContent sx={{ textAlign: "center", py: 2 }}>
            ...
            >
              Are you sure you want to enable partner{" "}
              <strong style={{ color: "#f97316" }}>"{enableDialog.partnerName}"</strong>?
              <br />
          ...
          <DialogActions sx={{ justifyContent: "center", gap: 2, pb: 2 }}>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              ...
              >
                {enablingPartner ? "Enabling..." : "Enable Partner"}
              </Button>
            </motion.div>
          </DialogActions>
        </Dialog>

        {/* Disable Partner Dialog */}
        <Dialog
        ...
        >
          ...
          <DialogContent sx={{ textAlign: "center", py: 2 }}>
            ...
            >
              ...
              <strong style={{ color: "#dc2626" }}>"{disableDialog.partnerName}"</strong>?
              <br />
          ...
          <DialogActions sx={{ justifyContent: "center", gap: 2, pb: 2 }}>
            ...
            <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
              ...
              >
                {disablingPartner ? "Disabling..." : "Disable Partner"}
              </Button>
            </motion.div>
          </DialogActions>
        </Dialog>
