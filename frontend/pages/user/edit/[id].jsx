...
export default function ProfileForm() {
  ...
  const [isFromEditPage, setIsFromEditPage] = useState(false)
  ...
  // Check if user came from edit page
  useEffect(() => {
    if (router.asPath.includes('/edit/')) {
      setIsFromEditPage(true);
    }
  }, [router.asPath]);
  ...
  // Prevent navigation if from edit page
  ...
  }, [isFromEditPage, handleBeforeUnload]);
              ...
              <div className="flex flex-wrap gap-2 border-b mt-10">
                {tabs.map((tab) => (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (isFromEditPage && activeTab === "personal") {
                        // Check if required fields are filled
                        const personalData = watch();
                        const requiredFields = ['name', 'email', 'phone', 'nationality', 'state', 'location', 'pincode'];
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2"
                              />
                              {errors.careerHistory?.[index]?.startDate && (
                                <p className="mt-1 text-sm text-red-500">{errors.careerHistory[index].startDate.message}</p>
                              )}
                            </div>
                              ...
                              <div className="flex-1">
                                ...
                                <input
                                  ...
                                  {...register(`careerHistory.${index}.endDate`, {
                                    validate: (value) => {
                                      if (watch(`careerHistory.${index}.isCurrentPosition`)) return true;
                                      ...
                                      const startDate = watch(`careerHistory.${index}.startDate`);
                                ...
                                />
                                {errors.careerHistory?.[index]?.endDate && (
                                  <p className="mt-1 text-sm text-red-500">{errors.careerHistory[index].endDate.message}</p>
                                )}
                              </div>
                              <div className="mt-7">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                              ...
                              {errors.certifications?.[index]?.issueDate && (
                                <p className="mt-1 text-sm text-red-500">{errors.certifications[index].issueDate.message}</p>
                              )}
                            </div>
                            <div className="flex items-start gap-4">
                              <div className="flex-1">
                                ...
                                <input
                                  ...
                                  {...register(`certifications.${index}.expirationDate`, {
                                    validate: (value) => {
                                      if (watch(`certifications.${index}.noExpiration`)) return true;
                                      if (!value) return "Expiration date is required unless marked as no expiration";
                                      const selectedDate = new Date(value);
                                      const today = new Date();
                                      today.setHours(23, 59, 59, 999); // End of today
                                      if (selectedDate > today) {
                                        return "Expiration date cannot be in the future";
                                      }
                                      const issueDate = watch(`certifications.${index}.issueDate`);
                                      if (issueDate && selectedDate <= new Date(issueDate)) {
                                        return "Expiration date must be greater than issue date";
                                      }
                                      return true;
                                    }
                                  })}
                                ...
                                />
                                {errors.certifications?.[index]?.expirationDate && (
                                  <p className="mt-1 text-sm text-red-500">{errors.certifications[index].expirationDate.message}</p>
                                )}
                              </div>
                              <div className="mt-7">
                                <label className="flex items-center gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    if (isFromEditPage) {
                      ...
                      if (missingFields.length > 0) {
                        toast.error(`Please fill in all required fields before leaving: ${missingFields.join(', ')}`);
                        return;
                      }
                    }
                    router.push(`/user/${user?.unifiedUser?.id}`);
                  }}
