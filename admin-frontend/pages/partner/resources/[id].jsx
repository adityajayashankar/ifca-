...
const EditResource = () => {
  ...
  const [searchSession, setSearchSession] = useState("");
    ...
    const partnerCommunities = allCommunities?.filter(
    (com) => com.creatorId === user?.unifiedUser?.id
  ) || [];

  const partnerSessions = allSessions?.filter(
    (session) => session.creatorId === user?.unifiedUser?.id
  ) || [];
  ...
  useEffect(() => {
    if (partnerCommunities && resourceData.communityArr) {
      const available = partnerCommunities.filter(
        community => !resourceData.communityArr.some(selected => selected.id === community.id)
      );
      setAvailableCommunities(available);
    }
  }, [partnerCommunities, resourceData.communityArr]);

  useEffect(() => {
    if (partnerSessions && resourceData.sessionArr) {
      const available = partnerSessions.filter(
        session => !resourceData.sessionArr.some(selected => selected.id === session.id)
      );
      setAvailableSessions(available);
    }
  }, [partnerSessions, resourceData.sessionArr]);
