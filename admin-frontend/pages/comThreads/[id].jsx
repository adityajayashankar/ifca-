...
const ComThreads = ({ blogId, sessionId, replyPostId }) => {
  ...
  return (
    <>
      {/* Breadcrumbs */}
      <div className="bg-white border-b border-gray-200 mb-4 max-h-[calc(100vh-72px)] overflow-y-auto scrollbar-hide ">
        {/* <div className="max-w-[1920px] mx-auto px-4 py-3 flex items-center space-x-2 text-sm">
          ...
          <Link href="/communities" underline="hover" sx={{ color: 'orange.700', fontWeight: 500 }}>
            Communities
          </Link>
          <NavigateNextIcon sx={{ fontSize: 18, color: 'orange.300' }} />
          <span className="text-orange-900 font-semibold">{thisCommunity[0]?.title}</span>
          <NavigateNextIcon sx={{ fontSize: 18, color: 'orange.300' }} />
          <span className="text-orange-900 font-semibold">Threads</span>
        </div> */}
        <ComThreadLayout
          blogId={blogId}
          sessionId={sessionId}
