...
export default function AnnouncementActivity({ activity, activityData, isModerator }) {
  ...
  // Memoize postId to ensure it doesn't change on every render
  const postId = useMemo(() => activityData?.postId, [activityData?.postId]);
  ...
  useEffect(() => {
    ...
    if (postId) {
      ...
      pollingIntervalRef.current = setTimeout(() => {
        if (isMountedRef.current && !isPollingRef.current && postId) {
  ...
  const fetchPostData = async (showLoading = true) => {
    ...
    try {
      ...
      const response = await api.get(`/thread/${postId}`, config);
      ...
      if (response.data && response.data.post) {
        ...
        if (likesChanged && user?.unifiedUser?.id) {
          ...
          setLikes({ [postId]: userLiked || false });
          previousLikesRef.current = newLikes;
        } else if (user?.unifiedUser?.id && !likes[postId]) {
  ...
  const handleLike = async () => {
    if (!user?.unifiedUser?.id || !postId) {
      toast.error('Please login to like announcements');
      return;
    }

    try {
      const currentLikeState = likes[postId] || false;
      const newLikeState = !currentLikeState;

      // Optimistic update
      setLikes({ [postId]: newLikeState });
