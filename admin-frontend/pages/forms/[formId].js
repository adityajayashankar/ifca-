...
const UserFormView = () => {
  ...
  // New state for handling previous responses
  ...
  const handleAnswerChange = (qid, value) => {
    if (hasResponded) return; // Prevent changes if already submitted
  ...
  const handleFileUpload = async (qid, file) => {
    ...
    try {
      ...
      // 3. Store fileUrl in answers
      setAnswers(prev => ({ ...prev, [qid]: fileUrl }));
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    const missingFields = formData?.Question?.filter(
      (question) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 overflow-x-hidden w-full" style={{ fontFamily: 'Roboto, Helvetica Neue, Arial, sans-serif' }}>
      <div className="w-full max-w-2xl mx-auto pt-8 pb-16 px-2 sm:px-4 md:px-6 lg:px-8 overflow-x-hidden">
        {/* Form Header */}
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border-t-4 w-full max-w-2xl mx-auto" style={{ borderTopColor: accentColor }}>
          ...
          {formData.Question?.[0]?.description && (
            <div className="w-full text-sm text-gray-600 border-b border-gray-200 pb-2 mb-2">
              {formData.Question[0].description}
            </div>
          )}
          {formData.isCommunitySpecific && formData.communityData?.length > 0 && (
            <div className="mb-2 text-sm text-orange-700">
              Community: {formData.communityData.map(c => c.communityName).join(', ')}
            </div>
          )}
          ...
          <div className="text-xs text-gray-500 mt-2">
            Form ID: {formId} | Questions: {formData?.Question?.length || 0}
          </div>
        </div>
        {/* Questions */}
        <form onSubmit={handleSubmit}>
          {formData?.Question && Array.isArray(formData.Question) && formData.Question.length > 0 ? (
            <div>
              {console.log('Rendering questions:', formData.Question.length, 'Has responded:', hasResponded, 'Previous responses:', previousResponses)}
                      ...
                      case 'shortText':
                        return (
                          <input
                            ...
                            value={hasResponded ? previousResponses[question.id] || '' : answers[question.id] || ''}
                            onChange={e => !hasResponded && handleAnswerChange(question.id, e.target.value)}
                            ...
                            {question.options.map((option, optIndex) => (
                              <label key={optIndex} className="flex items-center gap-2 cursor-pointer">
                                <input
                                  ...
                                  onChange={() => !hasResponded && handleAnswerChange(question.id, option)}
                        ...
                        return (
                          <div>
                            <input
                              ...
                              required={question.isRequired && !answers[question.id]}
                            />
                            ...
                            {!hasResponded && answers[question.id] && (
                              <div className="text-xs text-green-600 mt-1">
                                File uploaded: <a href={answers[question.id]} target="_blank" rel="noopener noreferrer" className="underline">View file</a>
                              </div>
                            )}
                          </div>
                        );
                          ...
                          <div className="flex gap-1">
                            {[1,2,3,4,5].map(star => (
                              <span
                                ...
                                onClick={() => !hasResponded && handleAnswerChange(question.id, star)}
            ...
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-400 mt-2">Form data: {JSON.stringify(formData, null, 2)}</p>
            </div>
          )}
          {!hasResponded ? (
            <button
