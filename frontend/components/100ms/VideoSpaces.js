    ...
    <div className={`flex items-center justify-center w-full h-full commScroll`}>
      <div className={`flex relative items-center justify-center flex-col w-full h-full bg-gray-800 overflow-hidden transition-all duration-300 rounded-lg border-2 ${
        ...
        {videoEnabled ? (
          ...
          <div className={`w-full ${pinned?.status ? "h-full" : "h-[160px]"} flex items-center justify-center bg-gray-800`}>
            <div className="text-center">
              <div className={`${pinned?.status ? "w-20 h-20" : "w-12 h-12"} mx-auto mb-3 rounded-full bg-gray-700 flex items-center justify-center`}>
                <span className={`text-gray-400 ${pinned?.status ? "text-2xl" : "text-lg"} font-semibold`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/90 to-transparent p-3 rounded-b-lg">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 flex-1 min-w-0">
              {isHost && (
                <span className="flex-shrink-0 bg-orange-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wide">
                  Host
                </span>
              )}
              <span className={`font-semibold text-sm ${isSpeaking ? 'text-green-400' : 'text-white'} truncate`}>
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              {audioEnabled === false && (
                <div className="bg-red-500 rounded-full p-1 flex-shrink-0">
                  <BiMicrophoneOff className="w-3 h-3 text-white" />
                </div>
              )}
              {videoEnabled === false && (
                <div className="bg-gray-600 rounded-full p-1 flex-shrink-0">
                  <BiVideoOff className="w-3 h-3 text-white" />
                </div>
              )}
            </div>
          </div>
