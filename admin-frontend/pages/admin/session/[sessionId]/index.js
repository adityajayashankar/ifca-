...
import {
  ...
  setOneSession,
  setSelectedSession,
  setSelectedSessionByIdWithApliedSpeakers,
...
function SessionPage() {
  ...
  const handleExpertNavigate = (e) => {
    ...
    router.push(`/admin/expert/${creator?.id}`);
  };


  // useEffect(() => {
  //   if (!isNaN(router?.query["sessionId"])) {
  //     dispatch(
  //       setSelectedSessionByIdWithApliedSpeakers(router?.query["sessionId"])
  //     );
  //   }
  // }, [router, visible]);
              ...
              <section className="flex flex-col gap-6 p-0 w-full lg:w-[35%] lg:sticky lg:top-24 bg-transparent z-10 h-fit">
                <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-6">
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <Link href={`/admin/session/${router.query["sessionId"]}/analytics`} passHref>
                    ...
                    <Link href={`/admin/session/${router.query["sessionId"]}/userview`} passHref>
                      <button className="relative overflow-hidden group bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-500 hover:to-orange-600 text-white px-4 py-2 rounded-xl font-medium flex items-center gap-2 shadow focus:ring-2 focus:ring-orange-300">
                        <div className="absolute top-0 left-0 w-full h-full bg-white/20 transform -skew-x-12 -translate-x-full group-hover:translate-x-[150%] transition-transform duration-700"></div>
                        <span className="relative z-10">User Page</span>
                      </button>
                    </Link>
                    <Link href={`/admin/session/create/${router.query["sessionId"]}`} passHref>
                      <button className="relative overflow-hidden group bg-gradient-to-r from-orange-600 to-orange-500 hover:from-orange-500 hover:to-orange-600 text-white px-4 py-2 rounded-xl font-medium flex items-center gap-2 shadow focus:ring-2 focus:ring-orange-300">
                        <div className="absolute top-0 left-0 w-full h-full bg-white/20 transform -skew-x-12 -translate-x-full group-hover:translate-x-[150%] transition-transform duration-700"></div>
              ...
              <section className="flex flex-col gap-8 p-0 w-full lg:w-[65%] lg:overflow-y-auto lg:max-h-[calc(100vh-7rem)]">
                              ...
                              {date?.speaker?.name ? (
                                <button
                                  ...
                                  onClick={e => {
                                    ...
                                    router.push(`/admin/expert/${date.speaker?.id}`);
                                  }}
                                >
                                  {date.speaker.name}
                ...
                <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
                  ...
                  {!selectedSession?.resources || selectedSession.resources.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
                      ...
                      <p className="text-gray-500 max-w-sm">
                        Add helpful materials, links, and resources for your session participants.
                      </p>
                      <Link href={`/admin/session/create/${router.query["sessionId"]}`} passHref>
                        <button className="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-orange-600 rounded-lg hover:bg-orange-700 transition-colors duration-200">
                        <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {selectedSession.resources.map((resource, index) => (
                        <div
                          key={index}
                          className="flex flex-col gap-3 p-4 border border-gray-200 rounded-xl bg-gradient-to-br from-orange-50 to-orange-100/50 hover:from-orange-100 hover:to-orange-200/50 transition-all duration-300 shadow-sm"
                        >
                          <div className="flex items-start justify-between">
                            <h4 className="text-base font-semibold text-gray-900 line-clamp-2">{resource.name}</h4>
                            <div className="flex gap-2">
                              {resource.isPreSession && (
                                <span className="px-2 py-1 text-xs font-medium text-blue-700 bg-blue-100 rounded-full">Pre-Session</span>
                              )}
                              {resource.isPostSession && (
                                <span className="px-2 py-1 text-xs font-medium text-green-700 bg-green-100 rounded-full">Post-Session</span>
                              )}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            ...
                            <a
                              ...
                              className="text-sm text-orange-600 hover:text-orange-700 underline truncate"
                            >
                              {resource.link}
                            </a>
                          </div>
                ...
                <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
                  ...
                  <div className="flex flex-col items-center justify-center py-8 px-4 text-center">
                    ...
                    <p className="text-gray-500 max-w-sm">
                      Coupon management feature will be available soon. Stay tuned for special discount options!
                    </p>
                    <div className="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-orange-600 bg-orange-50 rounded-lg border border-orange-200">
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                ...
                {/* Speaker Requests Section - Coming Soon */}
                <div className="bg-white rounded-2xl shadow p-6 flex flex-col gap-4">
                  <p className="text-lg font-bold flex items-center gap-2 text-orange-700">
                    ...
                    <h3 className="text-lg font-semibold text-gray-900 mb-2">Coming Soon</h3>
                      <p className="text-gray-500 max-w-sm">
                      Speaker request management feature will be available soon. Experts will be able to express interest in your sessions!
                      </p>
                    <div className="mt-4 inline-flex items-center px-4 py-2 text-sm font-medium text-orange-600 bg-orange-50 rounded-lg border border-orange-200">
                      <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
